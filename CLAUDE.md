...
**Appointments are a SHARED table with the public clinic website — not patient-linked.** The clinic's separate public website (its own repo) lets customers book online and INSERTs into a Postgres table named **`appointments`** that lives in the **same Neon database** this dashboard uses (one DB; `DATABASE_URL` points at it). The dashboard's Prisma `Appointment` model `@@map`s to that table ([schema.prisma](server/prisma/schema.prisma)): `BigInt` id, `appointment_date` (`@db.Date`), `appointment_time` as a **TEXT slot label** (e.g. `"04:30 PM"`), lowercase `status` (`pending|confirmed|cancelled`), and `source` (`website` | `manual` for front-desk entries | `lock` for staff slot-locks). **Do not change those column names/types — the website depends on them.** Bookings carry only a name + contact (no `patientId`), so patient cards no longer show a "next appointment" (`serializePatient` sets `nextAppt`/`apptTime`/`procedure` to `null` and derives `lastVisit` from the latest prescription). The fixed 10-slot/day system is the single source of truth in [server/src/lib/slots.ts](server/src/lib/slots.ts) (mirrored for the picker as `APPOINTMENT_SLOTS` in [src/data.ts](src/data.ts)) — it must stay identical to the website's slots. Bookings do **not** reserve a slot — multiple patients may book the same time; only a staff **lock** does. A partial unique index `uniq_active_lock (appointment_date, appointment_time) WHERE source='lock' AND status <> 'cancelled'` keeps at most one active lock per slot (replaces the old `uniq_active_slot`); [server/src/routes/appointments.ts](server/src/routes/appointments.ts) catches its violation (Postgres `23505` / Prisma `P2002`) → HTTP 409. `GET /api/appointments/availability?date=` reports per-slot `{ time, bookingCount, locked, lockId }` for that day.

**Overbooking model:** website bookings no longer reserve a slot — multiple patients can book the same time. Schedule shows each slot's live **booking count** and flags it **"full"** past a clinic threshold (`ClinicSettings.slotFullAt`, default 5, doctor-editable in Settings → Scheduling via `GET/PUT /api/settings/clinic`), but **locking stays a manual click**. **Booking creation is off the dashboard:** "New Appointment" opens the public booking site (`https://drashraf.vercel.app/appointment`) in a new tab (the `POST /api/appointments` manual-create route still exists but is UI-unused). **Slot locks:** staff lock a slot via `POST /api/appointments/locks` `{date,time}` (and `DELETE /api/appointments/locks/:id`), writing a sentinel `source='lock'`/`status='confirmed'` row. Only a lock makes a slot unavailable — on the public site too: the website's availability returns only lock rows and its POST refuses to insert when a lock exists. Locks are filtered OUT of the bookings list (`source != 'lock'`) and the stats counts; a `source='lock'` guard on the delete route ensures it can never remove a real booking.
...
- `Schedule.tsx` — day view of the shared `appointments` table: website + front-desk bookings with a Website/Manual source badge, confirm/cancel/delete, and a status filter. **Booking creation is NOT done here** — "New Appointment" opens the public booking site in a new tab. Above the list, a **Time Slots** grid (`useSlotAvailability` + `useClinicSettings` + `useLockSlot`/`useUnlockSlot`) shows each slot's booking count, flags "full" past the clinic threshold, and lets staff lock/unlock a slot to stop new website bookings. `Records.tsx` / `Billing.tsx` / `Settings.tsx` — prescription archive, invoices+payments, and account/appearance/staff (+ doctor-only **Scheduling** = the "full-at" threshold) management.
