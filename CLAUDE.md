...
- **Express app** is built in [server/src/app.ts](server/src/app.ts) (`createApp()`) and run two ways: locally via [server/src/index.ts](server/src/index.ts) (`app.listen`), and on Vercel via [api/index.ts](api/index.ts) (`serverless-http`). Vite proxies `/api` → `localhost:3001` in dev (see [vite.config.ts](vite.config.ts)).
...
- **Serializers** in [server/src/lib/serialize.ts](server/src/lib/serialize.ts) convert Prisma rows into the exact shapes the frontend expects (e.g. `status` → `{key, chip}`, derived `lastVisit`/`nextAppt`/`balance`). Change these if you change an API response shape — keep them in sync with `src/api/types.ts`.
...
- `Schedule.tsx` / `Records.tsx` / `Billing.tsx` / `Settings.tsx` — appointment calendar, prescription archive, invoices+payments, and account/appearance/staff management.
