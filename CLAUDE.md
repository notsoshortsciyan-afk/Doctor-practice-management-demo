...
- `Schedule.tsx` — day view of the shared `appointments` table: website + front-desk bookings with a Website/Manual source badge, confirm/cancel/delete, status filter, and a manual-create form whose slot picker disables already-booked slots (`useSlotAvailability`). `Records.tsx` / `Billing.tsx` / `Settings.tsx` — prescription archive, invoices+payments, and account/appearance/staff management.
